...
import com.marlem.mixm.exoplayer.State.*
...
class FirebaseMusicSource @Inject constructor(
    ...
    //function to convert as media items
    ...
    set(value){
        if(value == STATE_INITIALIZED || value == STATE_ERROR) {
            synchronized(onReadyListeners) {
                field = value //represent the update the new value
    ...
    fun whenReady(action:(Boolean)->Unit):Boolean{
        if(state == STATE_CREATED || state == STATE_INITIALIZED){
            ...
            return false //music player not ready
        }else{
            action(state == STATE_INITIALIZED)
                return true //music player is ready
        }
    }
}
